...
import KidEarth from "../../assets/images/earth.png";
...
const Mission3 = ({ onNext, onGoBack }) => {
  ...
  const applyChanges = () => {
    if (workerHappiness >= 100) {
      setShowSuccess(true);
    } else {
      alert(
        `Worker happiness is ${workerHappiness}%. Slide both to 100% to continue!`
      );
    }
  };
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-8 w-full">
            ...
            <div className="flex items-center bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
              ...
              <div className="sm:text-lg text-sm font-medium text-[#1E1E1E] font-[Inter] bg-[#F8F9FA] px-6 py-4 flex items-center min-w-[200px]">
                <span className="sm:text-xl text-sm font-bold text-[#4CAF50]">
                  {workerHappiness}%
                </span>
                <span className="sm:text-sm text-xs text-gray-500 ml-2 font-normal">
                  (Let's improve it!)
                </span>
              </div>
            </div>
              ...
              <div className="w-full max-w-xs sm:max-w-sm mb-3 sm:mb-4 relative">
                <div className="relative">
                  <input
                    ...
                    onChange={(e) =>
                      handleWorkplaceChange(parseInt(e.target.value))
                    }
                    className="w-full h-3 rounded-full appearance-none cursor-pointer slider bg-transparent"
                    style={{
      ...
      <Modal
        ...
        title={
          <>
            You Did it!
            <br />
            You’re a Sustainable Fashion Hero!
          </>
        }
        subtitle="Great job helping Lina make better fashion choices!"
        message={
          <div className="text-[#616161] font-['Inter']">
            <div>
              You learned how to{" "}
              <span className="font-semibold">Reduce waste</span>
            </div>
            <div className="font-semibold">
              Pick eco-friendly fabrics
            </div>
            <div>Support kind and safe workplaces</div>
          </div>
        }
        imageSrc={KidEarth}
        actions={[
